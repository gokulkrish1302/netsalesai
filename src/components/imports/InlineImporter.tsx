...
export function InlineImporter() {
  ...
  const handleFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      setResult(parsed);
      if (parsed.accounts.length === 0) toast.error("No valid accounts found");
      else toast.message("Review the preview, then populate to the tool.");
    } catch (err) {
  ...
  function populate() {
    ...
    toast.success(`Populated ${result.accounts.length} account${result.accounts.length === 1 ? "" : "s"} to the tool`);
    reset();
  }
      ...
      <div className="app-card space-y-4 p-5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-medium">
                {result.accounts.length} of {result.totalRows} rows ready · not yet added
              </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2">
          <p className="text-xs text-muted-foreground">
            Nothing has been added yet. Confirm to populate these accounts into the tool.
          </p>
          <div className="flex gap-2">
            <Button variant="ghost" className="pill" onClick={reset}>Cancel</Button>
            <Button className="pill px-5" disabled={result.accounts.length === 0} onClick={populate}>
              Populate {result.accounts.length} account{result.accounts.length === 1 ? "" : "s"} to tool
            </Button>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <div className="text-center">
          ...
          <div className="mt-0.5 text-xs text-muted-foreground">
            .xlsx, .xls, or .csv · up to 5 MB · you'll review before it's added
          </div>
        </div>
