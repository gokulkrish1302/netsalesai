...
function buildPrompt(row: Record<string, unknown>): string {
  ...
  return [
    ...
    `- Company size: ${row.company_size ?? "Unknown"}`,
    `- Sales rep: ${row.sales_rep ?? "Unassigned"}`,
    `- Annual revenue (USD): ${row.annual_revenue ?? "Unknown"}`,
    `- Last contact date: ${row.last_contact_date ?? "Unknown"}`,
    `- Pipeline stage: ${row.pipeline_stage ?? row.status ?? "Unknown"}`,
    `- Device model: ${row.device_model ?? "Unknown"}`,
    `- End of life: ${row.end_of_life ? "Yes" : "No"}`,
    `- Storage capacity (TB): ${row.storage_capacity_tb ?? "Unknown"}`,
